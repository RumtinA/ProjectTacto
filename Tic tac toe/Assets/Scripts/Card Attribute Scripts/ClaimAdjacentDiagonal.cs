...
public class ClaimAdjacentDiagonal : Attribute {
	...
	public void ActivateCode()
	{
		...
		if (boardSpaces [6].GetComponent<Spaces> ().GetIsTaken() && boardSpaces [6].GetComponent<Spaces> ().GetWhoHasIt() == GetActivePlayer()) // If space 7 is taken by the active player
		{
			if (!boardSpaces [4].GetComponent<Spaces> ().GetIsTaken()) // If space 4 is not taken
			{
				availability [4] = true; // Make space 4 available
